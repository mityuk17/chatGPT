...
def create_user(user_id):
    ...
    if cur.fetchall():
        return
    cur.execute(f'''INSERT INTO users(user_id, refs, daily_balance, current_balance, subscription_data, language) VALUES({user_id}, 0, 4000, 4000, 'en')''')
    conn.commit()
def add_ref(user_id):
    conn = sqlite3.connect('main.db')
    cur = conn.cursor()
